...
from app.services.task_service import attach_file_Service, create_task_service, delete_attachment_service, delete_task_service, get_analytics_service, get_task_service, get_task_list__service, open_attachment_service, update_task_service
...
async def delete_task(id:int=Query(...),  db:AsyncSession=Depends(get_db), current_user:User=Depends(get_current_user)):
    return await delete_task_service(id, db, current_user)
...
async def attach_file(task_id:int = Query(...), file : UploadFile=File(...), db:AsyncSession=Depends(get_db), current_user:User=Depends(get_current_user)):
    return await attach_file_Service(task_id, file, db, current_user)

@router.get("/attachment/open")
async def open_attachment(attachment_id:int = Query(...), db:AsyncSession=Depends(get_db), current_user:User=Depends(get_current_user)):
    return await open_attachment_service(attachment_id, db, current_user)

@router.delete("/attachment", status_code=200, response_model=APIResponse)
async def delete_attachment(attachment_id:int = Query(...), db:AsyncSession=Depends(get_db), current_user:User=Depends(get_current_user)):
    return await delete_attachment_service(attachment_id, db, current_user)

@router.post("/attachment/delete", status_code=200, response_model=APIResponse)
async def delete_attachment_post(attachment_id:int = Query(...), db:AsyncSession=Depends(get_db), current_user:User=Depends(get_current_user)):
    return await delete_attachment_service(attachment_id, db, current_user)
